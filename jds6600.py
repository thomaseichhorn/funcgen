...
# 2020/04/04: Thomas Eichhorn: Added Python 2.7-compatible output
...
class jds6600:
	...
	__wave=("Sine","Square","Pulse","Triangle","Partial Sine","CMOS","DC","Half-Wave","Full-Wave","Pos-Ladder","Neg-Ladder", "Noise", "Exp-Rise","Exp-Decay","Multi-Tone","Sinc","Lorenz")
	...
	__freqmultiply=(1,1,1,1/1000.0,1/1000000.0)
	...
	def __parsedata(self,reg,data,a):
		...
		try:
			(one,two)=data.decode("ascii").split("=")
		except ValueError:
	...
	def getfrequency_m(self,channel):
		...
		try:
			return((f1/100.0*self.__freqmultiply[f2],f2))
		except IndexError:
	...
	def getfrequency(self,channel):
		...
		try:
			return(f1/100.0*self.__freqmultiply[f2])
		except IndexError:
			# unexptected value of frequency multiplier
			raise UnexpectedValueError(f2)
		# end elsif
	...
	def measure_getall(self):
		...
		# return all
		return (freq_f / 10.0, freq_p / 1000.0, pw1/100.0, pw0/100.0, period/100.0, dutycycle/10.0)
	# end get freq-Lowres (measure)
	...
	def sweep_getstartfreq(self):
		...
		return self.__getdata(jds6600.SWEEP_STARTFREQ)/100.0
	# end get sweep - startfreq

	def sweep_getendfreq(self):
		# unit is  0.01 Hz -> divide by 100
		return self.__getdata(jds6600.SWEEP_ENDFREQ)/100.0
	# end get sweep - startfreq

	def sweep_gettime(self):
		# unit is  0.1 sec -> divide by 10
		return self.__getdata(jds6600.SWEEP_TIME)/10.0
	# end get sweep - startfreq
	...
	def pulse_getpulsewidth(self):
		...
		if multi==0: return time / 1000000000.0 # ns
		elif multi == 1: return time / 1000000.0 # us
		else:
	...
	def pulse_getamplitude(self):
		...
		return self.__getdata(jds6600.PULSE_AMPLITUDE)/100.0
	# end 
